...
CREATE TABLE IF NOT EXISTS editorial_projects (
  ...
  current_stage text DEFAULT 'ingesta' CHECK (current_stage IN ('ingesta', 'estructura', 'estilo', 'ortotipografia', 'maquetacion', 'revision_final')),
...
CREATE TABLE IF NOT EXISTS editorial_stages (
  ...
  status text DEFAULT 'pending' CHECK (status IN ('pending', 'queued', 'processing', 'review_required', 'approved', 'failed', 'completed')),
